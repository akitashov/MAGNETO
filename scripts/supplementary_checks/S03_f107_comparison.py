#!/usr/bin/env python3
"""
S03_f107_comparison.py — Compare SIF 771 nm associations for SII vs F10.7 cm radio flux.

F10.7 is a solar-activity proxy. The comparison uses identical observations and the
same fixed windows (21, 28 days) as the primary SII analysis.
"""
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="SII vs F10.7 comparison")
...
def main(argv=None) -> int:
    ...
    print("=" * 64)
    print("MAGNETO supplementary checks — SII vs F10.7 comparison")
    print("=" * 64)
    print(f"Windows: {windows}")

    print("\n[1/3] Building exposures ...")
    sii_exp = build_sii_exposure(windows)
    f107_exp = build_omni_exposure("f10_7_mean", windows, "f10_7")
    ...
    for method in args.methods:
        ...
        df = attach_exposure(df, f107_exp)

        all_exp_cols = [f"{p}_{w}d" for p in ("sii", "f10_7") for w in windows]
        df_clean = df.dropna(subset=all_exp_cols + [args.outcome])

        for exposure_prefix in ("sii", "f10_7"):
    ...
    df_out["exposure"] = df_out["sample_type"].apply(
        lambda x: "F10.7" if x.endswith("f10_7") else "SII"
    )

    from _Common import FILE_RES_HARMONIC, FILE_RES_CYCLIC, FILE_OMNI
    df_out = attach_provenance(
        df_out,
        stage="S03_f107_comparison.py",
        dataset_files=[FILE_RES_HARMONIC, FILE_RES_CYCLIC, FILE_OMNI],
    )

    out_csv = SUPPLEMENTARY_CHECKS_RESULTS / cfg["outputs"]["f107_comparison_csv"]
    atomic_write(df_out, out_csv)
    print(f"\n  Written: {out_csv}")
    print(f"  Rows: {len(df_out)}")

    if args.figure:
        make_f107_figure(df_out, cfg)

    return 0


def make_f107_figure(df: pd.DataFrame, cfg: dict) -> None:
    ...
    for i, w in enumerate(windows):
        for j, method in enumerate(methods):
            ...
            if temp.empty:
                ...
                continue
            order = [c for c in temp_order if c in temp["temp_bin_label"].values]
            pivot = temp.pivot_table(
                index="temp_bin_label", columns="exposure", values="spearman_rho", aggfunc="first"
            ).reindex(order)
            pivot.plot(kind="bar", ax=ax, color=["steelblue", "forestgreen"])
    ...
    fig.suptitle("SIF 771 nm: SII vs F10.7 across temperature classes")
    out_png = SUPPLEMENTARY_CHECKS_FIGURES / cfg["outputs"]["f107_comparison_fig"]
    savefig(out_png, fig)
